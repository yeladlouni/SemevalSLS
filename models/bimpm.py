...
class BiMPM(nn.Module):
    def __init__(self, config):
        ...
        self.n_perspectives = config.n_perspectives
        self.n_hidden = config.n_hidden
        self.n_classes = config.n_classes
        self.dropout = config.dropout

        # each word represented with d-dimensional vector with two components
        self.d = config.d_embed
        # l is the number of perspectives
        self.l = config.n_perspectives

        self.embeddings = nn.Embedding(self.n_embed, self.d_embed)
        self.embeddings.weight = nn.Parameter(config.weights)
        ...
        self.m_full_forward_W = nn.Parameter(torch.rand(self.l, self.n_hidden))       # W^1 in paper
        self.m_full_backward_W = nn.Parameter(torch.rand(self.l, self.n_hidden))      # W^2 in paper
        self.m_maxpool_forward_W = nn.Parameter(torch.rand(self.l, self.n_hidden))    # W^3 in paper
        self.m_maxpool_backward_W = nn.Parameter(torch.rand(self.l, self.n_hidden))   # W^4 in paper
        self.m_attn_forward_W = nn.Parameter(torch.rand(self.l, self.n_hidden))       # W^5 in paper
        self.m_attn_backward_W = nn.Parameter(torch.rand(self.l, self.n_hidden))      # W^6 in paper
        self.m_maxattn_forward_W = nn.Parameter(torch.rand(self.l, self.n_hidden))    # W^7 in paper
        self.m_maxattn_backward_W = nn.Parameter(torch.rand(self.l, self.n_hidden))   # W^8 in paper
        ...
        self.prediction_layer = nn.Sequential(
            nn.Linear(4*self.n_hidden, 2*self.n_hidden),
            nn.Tanh(),
            nn.Dropout(config.dropout),
            nn.Linear(2*self.n_hidden, self.n_classes),
            nn.LogSoftmax(1)
        )
    ...
    def matching_strategy_full(self, v1, v2, W):
        ...
        if len(v2.size()) == 2:
            ...
            raise ValueError("Invalid v2 tensor size {v2.size()}")
    ...
    def forward(self, batch):
        ...
        sent1 = self.embeddings(batch.question)
        sent2 = self.embeddings(batch.qaquestion)
        ...
        s1_context_forward, s1_context_backward = torch.split(s1_context_out, self.n_hidden, dim=2)
        # s2_context_forward and backward are dimensions batch x seq_len_2 x n_hidden
        s2_context_forward, s2_context_backward = torch.split(s2_context_out, self.n_hidden, dim=2)
        ...
        matching_vector = torch.cat([s1_agg_h.transpose(1, 0), s2_agg_h.transpose(1, 0)], dim=1).view(-1, 4 * self.n_hidden)
